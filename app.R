...
dose_mod    <- get_rds("dose_mod.rds")
tanfloc_mod <- get_rds("tanfloc_mod.rds")
tanfloc_rem <- get_rds("tanfloc_rem.rds")


environment(tanfloc_rem)    <- .GlobalEnv
environment(dose_mod)    <- .GlobalEnv
environment(tanfloc_mod) <- .GlobalEnv
...
server <- function(input, output, session) {
  ...
  model_output <- eventReactive(input$actionButton,{  
    ...
    #Call for model
    Output2 <- tanfloc_mod(model = tanfloc_rem,
                          effluent = df[1,2],
                           turbidity = df[2,2],
